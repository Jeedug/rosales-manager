...
export default function Editor({
  ...
  const [packageItem, setPackageItem] = useState(editablePackage);


  const handleEdit = async () => {
    await fetch(
      "https://viajesrosales.netlify.app/api/packages/update",
      {
        method: "POST",
        body: http.Body.json({
    ...
    setAllPackages((allPacks) =>
      allPacks.map((pack) =>
        pack.id === editablePackage.id ? packageItem : pack
      )
    );
  };
  ...
  useEffect(() => {
    setPackageItem(editablePackage);
  }, [editablePackage]);
    ...
    return (
      <div className="flex flex-col items-start justify-start pt-10 border w-full px-10 ">
        ...
        <h2 className="text-[12px] text-gray-700 font-normal mr-40 mb-2">
          Edita los datos del paquete seleccionado.
        </h2>
        ...
        <div className=" w-full flex flex-col relative overflow-y-scroll">
          ...
          <div className="w-full border-b-2">
            ...
            {packageItem?.items?.map((item, index) => (
              <EditorItem setHasChanged={setHasChanged} key={index} item={item} index={index} setPackageItem={setPackageItem} packageItem={packageItem} />
            ))}
