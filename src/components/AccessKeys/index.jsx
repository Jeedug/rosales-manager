...
export default function AccessKeys({ selected }) {
  ...
  useEffect(() => {
    const getAllKeys = async () => {
      const data = await fetch(
        "https://viajesrosales.netlify.app/api/keys/read",
        {
          method: "POST",
          body: http.Body.json({
  ...
  const handleCreateKey = async () => {
    const data = await fetch(
      "https://viajesrosales.netlify.app/api/keys/create",
      {
        method: "POST",
        body: http.Body.json({
          key: settings.access.key,
    ...
    setAllKeys((allKeys) => [...allKeys, data.data]);
  };

  if (selected == "Llaves de acceso") {
    return (
      <>
              ...
              <tbody>
                ...
                {status !== 200 ? (
                  <tr
                    ...
                    className="odd:bg-white  border-b text-black hover:bg-gray-100 cursor-pointer"
                  >
                    <th
                      scope="row"
